...
X, X_prime, y = extract_df_from_ds(dataset_orig)


def single_trial(predictor='kernel', C=15, printflag=True, gamma=0.1, max_iter=10):
    if predictor == 'kernel':
    ...
    fair_clf.fit(dataset_orig)


def multiple_comparision(gamma_list=tuple([0.01]), max_iter_list=tuple([5]), C=15):
...
def multiple_pareto(gamma_list=[0.002, 0.005, 0.01, 0.02, 0.05, 0.1], save_results=True):
