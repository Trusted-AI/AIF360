...
class LimeEncoder(Transformer):
    ...
    def fit(self, dataset):
        """
        Take an aif360 dataset and save all relevant metadata as well as mappings needed to 
        transform/inverse_transform the data between aif360 and lime
    ...
    def transform(self, aif360data):
        """
        Take aif360 data array and return data array that is lime encoded (numeric array in which categorical features are NOT one-hot-encoded)
    ...
    def inverse_transform(self, limedata):
        """
        Take data array that is lime encoded (that is, lime-compatible data created by this class from a given aif360 dataset) and return data array consistent with the original aif360 dataset
            ...
            for ind, feature in enumerate(self.s_feature_names):
                if ind in self.s_categorical_features:    #s_catgorical_features has list of indexes into s_feature_names for categorical features
                    if feature in self.s_protected_attribute_names:
                        ...
                        aif360data[rw, self.s_feature_names_with_one_hot_encoding.index(new_feature)] = 1.0   # categorical feature
