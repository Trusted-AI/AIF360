...
test_that("running dataset test", {
  ...
  expect_equal(cm$generalized_false_positive_rate(privileged=TRUE), 0.2222)
  ...
  expect_equal(cm$generalized_true_negative_rate(privileged=TRUE), 0.778, tolerance=-0.000222)
  expect_equal(cm$generalized_true_negative_rate(privileged=FALSE), 0.428, tolerance=0.000571)
  ...
  expect_equal(cm$positive_predictive_value(), 0.143, tolerance=-0.000143)
  expect_equal(cm$power(), 1)
  expect_equal(cm$precision(), 0.143, tolerance=-0.000143)
  ...
  expect_equal(cm$statistical_parity_difference(), 0.172, tolerance=-0.000283)
  expect_equal(cm$theil_index(), 0.220, tolerance=0.00188)
