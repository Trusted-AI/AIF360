...
class AdversarialDebiasing(Transformer):
    ...
    def _classifier_model(self, features, features_dim, keep_prob):
        ...
        with tf.variable_scope("classifier_model"):
            ...
            W2 = tf.get_variable('W2', [self.classifier_num_hidden_units, 2],
                                 initializer=tf.contrib.layers.xavier_initializer())
    ...
    def _adversary_model(self, pred_logits, true_labels):
        ...
        with tf.variable_scope("adversary_model"):
            ...
            # Apply softmax to get the predicted class probabilities
            pred_protected_attribute_label = tf.nn.softmax(pred_protected_attribute_logit)
    ...
    def fit(self, dataset):
        ...
        with tf.variable_scope(self.scope_name):
            ...
            pred_labels_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.true_labels_ph, logits=pred_logits))
            ...
            if self.debias:
                ...
                pred_probs = tf.nn.softmax(pred_protected_attributes_logits)
                #pred_protected_attributes_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.protected_attributes_ph, logits=pred_protected_attributes_logits))
                pred_protected_attributes_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.protected_attributes_ph, logits=pred_protected_attributes_logits))
            ...
            for epoch in range(self.num_epochs):
                ...
                for i in range(num_train_samples//self.batch_size):
                    ...
                    if self.debias:
                        ...
                        #print(probs)
                        #print(self.sess.run(pred_probs, feed_dict=batch_feed_dict))
                        if i % 200 == 0:
    ...
    def predict(self, dataset):
        ...
        while samples_covered < num_test_samples:
            ...
            pred_probs += self.sess.run(self.pred_labels, feed_dict=batch_feed_dict)[:, 1].tolist()
            #pred_probs+= self.sess.run(self.pred_probs, feed_dict=batch_feed_dict).tolist()
            #y_pred_cls = self.sess.run(self.y_pred_cls, feed_dict=batch_feed_dict)
            #print(y_pred_cls)
        ...
        dataset_new.probs = (np.array(pred_probs)).astype(np.float64).reshape(-1, 1)
        #dataset_new.pred_cls = (np.array(y_pred_cls)).astype(np.float64)
        #print(pred_probs)
